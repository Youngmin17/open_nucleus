...
(
    ...
    output [((1+MANT_WIDTH)*2+SIGN_WIDTH)*DATA_NUM-1:0] mant_out
    );
    ...
    wire [EXP_WIDTH-1:0] comp_6th_w [(DATA_NUM>>6)-1:0];
    ...
    begin : GENERATE_PRE_ALIGNMENT
        ...
        assign one_mant_w[i] = (zero[i]) ? {(SIGN_WIDTH+MANT_WIDTH){1'b0}} : {1'b1, mant_w[i]};
    end
    endgenerate
    ...
    generate
        ...
        begin : L5
            assign comp_5th_w[i] = (comp_4th_w[2*i] >= comp_4th_w[2*i+1]) ? comp_4th_w[2*i] : comp_4th_w[2*i+1];
        end
    endgenerate
    ...
    generate
        ...
        begin : L6
            assign comp_6th_w[i] = (comp_5th_w[2*i] >= comp_5th_w[2*i+1]) ? comp_5th_w[2*i] : comp_5th_w[2*i+1];
        end
    endgenerate

    assign comp_out = ((comp_6th_w[0] >= comp_6th_w[1]) ? comp_6th_w[0] : comp_6th_w[1]);

    generate
    for (i=0; i<DATA_NUM; i=i+1)
    begin : LEADING_BIT_MANTISSA_SHIFT
        exp_sub_mant_shift_fmat #(
