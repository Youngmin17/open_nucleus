...
`timescale 1ns / 1ps
module norm_round_concat
#(
    parameter MANT_WIDTH_IN  = 24,
    parameter EXP_WIDTH_IN   = 8,
    parameter MANT_WIDTH_OUT = 7,
    parameter DATA_WIDTH_OUT = 1 + EXP_WIDTH_IN + MANT_WIDTH_OUT
)
(
    ...
    localparam GUARD_POS  = MAG_WIDTH - 1 - MANT_WIDTH_OUT;
    ...
    wire [MANT_WIDTH_OUT-1:0] norm_mant;
    ...
    lzd_8bit_concat lzd_left(
        .d_in(dec_point_left),
        .d_out(zero_cnt_left)
    );

    lzd_15bit_concat lzd_right(
    ...
    assign norm_mant = shift_mant[MAG_WIDTH-1 -: MANT_WIDTH_OUT];
    ...
    assign guard_bit  = shift_mant[GUARD_POS];
    assign round_bit  = shift_mant[GUARD_POS-1];
    assign sticky_bit = |shift_mant[GUARD_POS-2:0];
    ...
    assign d_out = (exp_overflow) ? {sign, {EXP_WIDTH_IN{1'b1}}, {MANT_WIDTH_OUT{1'b0}}} :
                   ((exp_underflow) ? {sign, {EXP_WIDTH_IN{1'b0}}, {MANT_WIDTH_OUT{1'b0}}} :
                    {sign, exp, mant});
endmodule
